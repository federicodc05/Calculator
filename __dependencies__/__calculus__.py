...
def calculus():
    ...
    scroll = Scrollbar(calculus_screen, orient=HORIZONTAL)
    ...
    i_f.grid(row=4, column=2, rowspan=2)
    lim.grid(row=2, column=2)
    result.grid(row=6, column=0, columnspan=3)
    lap.grid(row=4, column=0, rowspan=2)
    exp.grid(row=4, column=1)
    fac.grid(row=5, column=1)
    scroll.grid(row=3, column=0, columnspan=2)
    scroll.configure(command=func.xview)
